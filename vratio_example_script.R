...
# 1. Simulate some data, and treat these as if these are the real data we want to estimate
#which parameters to use for the "real" data
real_params <- c(1,1,.2,.75,1,1)
names(real_params) <- c("v","a","ter","vratio","add_mean","add_sd")

#the function requires observations$rtcj as input for the simulations
observations <- data.frame(matrix(NA,nrow=890));observations$rtcj <- abs(rnorm(890))
#how many confidence levels do you want in your simulations?
conf_levels = 6
...
#some quick checks
mean(df$cor)
hist(df$rt);range(df$rt)
table(df$cj)

# 2. Now let's fit these data to find the parameters underlying these data
